...
export function ResearchPanel({ perplexicaUrl, caseTitle }: Props) {
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      {/* ── Tab: AI Research (Perplexica external link) ── */}
      {tab === "ai" && (
        <div className="flex flex-col h-full">
          {/* Quick search box — same API, opens results here */}
            ...
            <div className="flex gap-2">
              ...
              {perplexicaUrl && (
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="flex items-center gap-1 border border-[var(--border)] text-[var(--text-3)] hover:text-[var(--accent)] hover:border-[var(--accent)] text-xs rounded px-3 py-1.5 transition-colors whitespace-nowrap"
                >
                  <ExternalLink size={11} /> Perplexica
                </a>
              )}
            </div>
          </div>
          ...
          <div className="flex-1 overflow-y-auto px-3 pb-3 space-y-2">
            ...
            {!searched && !loading && (
              <div className="flex flex-col items-center justify-center h-32 gap-2 text-[var(--text-3)] px-4 text-center">
                <Sparkles size={24} className="opacity-30" />
                <p className="text-xs">พิมพ์คำถามแล้วกด Enter</p>
                {perplexicaUrl && (
                  <a
                    href={perplexicaUrl}
                    target="_blank"
                  ...
                  >
                    <ExternalLink size={10} /> เปิด Perplexica แบบเต็มหน้าจอ
                  </a>
                )}
              </div>
            )}
            {results.map((r, i) => (
              ...
              >
                ...
                <span className="text-[9px] text-[var(--accent)] truncate max-w-[180px] block">
                  {r.url.replace(/^https?:\/\//, "").split("/")[0]}
                </span>
              </a>
            ))}
