...
แพลตฟอร์มข่าวกรองสำหรับห้องข่าว — คัดกรองข่าวอัตโนมัติ สืบสวนเชิงลึก ตรวจสอบสื่อ จำลองสถานการณ์ และเฝ้าระวัง dark web บนเซิร์ฟเวอร์ของคุณเอง ใช้ AI ที่รันในเครื่อง ข้อมูลไม่รั่วออกนอกองค์กร
...
7. [Step 3 · ตรวจสอบสื่อ (Verify)](#step-3--ตรวจสอบสื่อ-verify)
...
ollama pull qwen3:14b         # สำหรับ brief/simulation (แนะนำ)
...
- Brief Model:   qwen3:14b  (แนะนำถ้า RAM พอ)
- Vision Model:  gemma4:12b (สำหรับตรวจสอบรูป/วิดีโอ)
```
...
| **Transcript** | faster-whisper | ถอดเสียงวิดีโอ ตรวจสอบเนื้อหา |
| **Visual Analysis** | Vision LLM | อธิบายสิ่งที่เห็น ตรวจหาสัญญาณ manipulation |
...
```bash
docker compose -f docker-compose.dev.yml up -d             # ขึ้น stack ทั้งหมด
docker compose -f docker-compose.dev.yml logs -f api        # tail API logs
docker compose -f docker-compose.dev.yml logs -f worker beat # Celery logs
docker compose -f docker-compose.dev.yml ps                 # สถานะ services

make test           # full test suite
make test-unit      # unit tests (ไม่ต้องการ DB)
make lint           # ruff + mypy + eslint
make migrate        # alembic upgrade head
make migrate-create msg="add_new_column"  # สร้าง migration ใหม่
